...
def kraus_to_superoperator(kraus_channel):
...
def kraus_to_choi(kraus_channel):
    so = kraus_to_superoperator(kraus_channel)
    choi = reshuffle(so)
...
class DilutedChannel:
    ...
    @property
    def choi(self):
        choi = (1 - self.p) * self.channel_a.choi + self.p * self.channel_b.choi
        return choi
...
class ConcatChannel(Channel):
    def __init__(self, channel_list, spam=None):
