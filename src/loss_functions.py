...
class ProbabilityMSE:
    ...
    def __call__(self, channel, input, target):
        ...
        loss = d**2 * tf.math.reduce_mean(tf.abs((output - target) ** 2))
...
class ProbabilityRValue:
    ...
    def __call__(self, channel, input, target):
        ...
        loss = 1 - (
            ...
            / tf.math.reduce_std(tf.abs(target)) ** 2
        )
...
class SpectrumDistance:
    ...
    def __init__(self, sigma=0.1, k=None, mode="density", remove_shift=False):
        self.sigma = sigma
        self.sigma_ = sigma
        self.k = k
    ...
    def __call__(self, channel, input, target):
        spectrum_target = target[0].get_spectrum(is_complex=False)[:-1]
        spectrum_model = channel_spectrum(channel).get_spectrum(is_complex=False)[:-1]

        self.not_connected = input
    ...
    def greedy_pair_distance(self, spectrum_a, spectrum_b):
        ...
        for i in range(len(spectrum_a)):
            self.not_connected[i].assign(True)


        spectrum_a = tf.unstack(spectrum_a)
        spectrum_b = tf.unstack(spectrum_b)
        for i, a in enumerate(spectrum_a):
            min_dist = tf.cast(np.inf, tf.float64)
            idx = 0
            for j, b in enumerate(spectrum_b):
                dist = tf.abs((a[0] - b[0]) ** 2 + (a[1] - b[1]) ** 2)
                if (dist < min_dist) and self.not_connected[j]:

                    min_dist = dist
                    idx = j

            self.not_connected[idx].assign(False)
    ...
    def greedy_pair_distance(self, spectrum_a, spectrum_b):
        ...
        for i, a in enumerate(tf.unstack(spectrum_a, axis=0)):
            min_dist = tf.constant(np.inf, dtype=tf.float64)
            idx = 0
            for j, b in enumerate(tf.unstack(spectrum_b)):
                dist = tf.abs((a[0] - b[0]) ** 2 + (a[1] - b[1]) ** 2)
