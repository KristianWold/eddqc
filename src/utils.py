...
def train_val_split(inputs, targets, ratio=0.9):
    N = targets.shape[0]
    indicies = tf.range(N)
    indicies = tf.random.shuffle(indicies)
    train_indicies = indicies[: int(ratio * N)]
    val_indicies = indicies[int(ratio * N) :]

    if isinstance(inputs, list):
        inputs_train = [tf.gather(inputs[0], train_indicies), tf.gather(inputs[1], train_indicies)]
        inputs_val = [tf.gather(inputs[0], val_indicies), tf.gather(inputs[1], val_indicies)]
    else:


        inputs_train = tf.gather(inputs, train_indicies)
        inputs_val = tf.gather(inputs, val_indicies)

    targets_train = tf.gather(targets, train_indicies)
    targets_val = tf.gather(targets, val_indicies)
