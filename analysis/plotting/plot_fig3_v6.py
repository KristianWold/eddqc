...
def loader(filename):
    object = pkl.load(open(filename, "rb"))
...
def plot_all(save=False):
    ...
    cax = fig.add_subplot(gs[15:35, 80:81])
    ...
    for i in range(2):
        ax = fig.add_subplot(
            gs[y0 : y0 + dy, 45 + interval + i * dx : 45 + (i + 1) * dx]
        )
        dist_axes.append(ax)
    for i in range(2):
        ax = fig.add_subplot(
            gs[
                ...
                45 + interval + i * dx : 45 + (i + 1) * dx,
            ]
        )
        dist_axes.append(ax)
    ag_dist = AxesGroup(4, 2, figs=fig, init=False)
    ...
    ax.text(0.6575, 0.215, "$r$", fontsize=FONTSIZE + 1, ha="left", va="center")
    ax.text(0.6505, 0.126, r"$\theta$", fontsize=FONTSIZE + 1, ha="left", va="bottom")
    ...
    r_baseline, a_baseline = loader("../baseline_comparison/data/integrable_scatter_baseline.pkl")
    r_intermediate, a_intermediate = loader("../baseline_comparison/data/integrable_scatter_intermediate.pkl")

    color_list = ["C1", "C3", "C6"]

    x, y = scatter_result[5]
    ax.plot([r_intermediate[0], x], 
           [a_intermediate[0], y], 
           color="C1", linestyle="--", linewidth=LW-0.5, zorder=-np.inf)
    
    x, y = scatter_result[20]
    ax.plot([r_intermediate[1], x], 
           [a_intermediate[1], y], 
           color="C3", linestyle="--", linewidth=LW-0.5, zorder=-np.inf)
    
    x, y = scatter_result[50]
    ax.plot([r_intermediate[2], x], 
           [a_intermediate[2], y], 
           color="C6", linestyle="--", linewidth=LW-0.5, zorder=-np.inf)


    # for color, x, y in zip(color_list, r_baseline, a_baseline):
    #     ax.errorbar(
    #     [x],                   # x-coordinate(s)
    #     [y],                   # y-coordinate(s)
    #     #xerr=[std1],
    #     #yerr=[std2],            # vertical error bar(s)
    #     linestyle="",          # no connecting line between points
    #     marker="d",            # draw each point as a circle
    ...
    for color, x, y in zip(color_list, r_intermediate, a_intermediate):
        ax.errorbar(
        [x],                   # x-coordinate(s)
        [y],                   # y-coordinate(s)
        #xerr=[std1],
        #yerr=[std2],            # vertical error bar(s)
        linestyle="",          # no connecting line between points
        marker="^",            # draw each point as a circle
        #markerfacecolor='none',
        mec='black',
        color=color,
        markersize=MS,         # size of the circle marker
    ...
    ag_dist.set_xlabel(
        r"$r$", fontsize=FONTSIZE+2, axes=[0, 1], sharex=1, labelpad=0.5
    )
    ag_dist.set_ylabel(r"$P(r)$", fontsize=FONTSIZE+1, axes=[0, 1], sharey=1)
    xs = [0, 0.5, 1]
    ys = [0.0, 1.5, 3.0]
    ag_dist.set_xticks(
        xs, xlim=xs, fontsize=FONTSIZE+1, sharex=1, xlim_pad_ratio=0.05, axes=[0, 1]
    )
    ag_dist.set_yticks(
        ys, ylim=ys, fontsize=FONTSIZE+1, sharey=1, ylim_pad_ratio=0.05, axes=[0, 1]
    )
    ...
    ag_dist.set_ylabel(r"$P(\theta)$", fontsize=FONTSIZE+1, axes=[2, 3], sharey=1)
    xs = [-np.pi, 0.0, np.pi]
    ys = [0, 0.2, 0.4]
    ag_dist.set_xticks(
        xs,
    ...
    ag_dist.set_yticks(
        ys, ylim=ys, fontsize=FONTSIZE+1, sharey=1, ylim_pad_ratio=0.05, axes=[2, 3]
    )
