...
def plot_all(save=False):
    ...
    # Make two cbar axes: top row and bottom row
    cax_top    = fig.add_subplot(gs[8:33, 91:93])
    cax_bottom = fig.add_subplot(gs[62:87, 91:93])
    ...
    norm_top = mcolors.Normalize(vmin=vmin, vmax=0.6)
    sm_top = mpl.cm.ScalarMappable(norm=norm_top, cmap=cmap)
    sm_top.set_array([])

    norm_bottom = mcolors.Normalize(vmin=vmin, vmax=2.5)
    sm_bottom = mpl.cm.ScalarMappable(norm=norm_bottom, cmap=cmap)
    sm_bottom.set_array([])
    ...
    cax_bottom.set_yticks([vmin, 2.5])
