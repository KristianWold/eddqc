...
def collect_data():
    ...
    for type in types:
        if type == "Integrable":
            ...
            name = path / "hist_nonintegrable_L=5.txt"
...
def plot_all(save=False):
    ...
    cax_ch = fig.add_axes([0.89, 0.14, 0.015, 0.30])   # adjust to your layout
    cbar_ch = fig.colorbar(
        mpl.cm.ScalarMappable(norm=norm_ch, cmap=cmap),
        cax=cax_ch,
        orientation="vertical",
        label="probability",
    )
    cbar_ch.set_ticks([vmin, vmax])
