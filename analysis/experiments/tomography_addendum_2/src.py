...
def prepare_input_string(config, return_mode="circuit", n_anc = 0):
    """Prepare a circuit that prepares a given input Pauli string.
    0 = |0>, 1 = |1>, 2 = |+>, 3 = |->, 4 = |i+>, 5 = |i->"""
    n = len(config)
    circuit = qk.QuantumCircuit(n + n_anc)
...
def prepare_output_string(config, return_mode="circuit", n_anc = 0):
    """Prepare a circuit that measures a given output Pauli string.
    0 = X, 1 = Y, 2 = Z"""
    n = len(config)

    q_reg = qk.QuantumRegister(n + n_anc)
...
def generate_circuit_sandwich(n=None,
    ...
    for i, j in zip(input_index_list, output_index_list):
        ...
        state_circuit = prepare_input_string(
            config1, return_mode="circuit", n_anc = n_anc
        )
        observable_circuit = prepare_output_string(
            config2, return_mode="circuit", n_anc = n_anc
        )
...
def prepare_SPAM_strings(n, n_anc = 0):

    circuit_list = []
    for i in range(6**n):
        config = numberToBase(i, 6, n)
        
        circuit = prepare_input_string(config, return_mode="circuit", n_anc = n_anc)
        circuit.barrier()
        circuit.add_register(qk.ClassicalRegister(n))
        circuit.measure(range(n), range(n))
        circuit_list.append(circuit)
...
def integrable_circuit_weak_coupling(n, L):
    theta_list = [np.random.uniform(-np.pi, np.pi, n) for i in range(L)]
    sqrt_iSWAP = iSwapGate().power(1 / 2)
    tuned_coup = iSwapGate().power(1/6)
    ...
    for i, theta in enumerate(theta_list):
        ...
        for j in range((n - offset) // 2):
            if j + offset != 2:  # different coupling strength on last qubit
                circuit.append(sqrt_iSWAP, [2 * j + offset, 2 * j + 1 + offset])
            else:
...
def integrable_circuit_no_coupling(n, L):
    theta_list = [np.random.uniform(-np.pi, np.pi, n) for i in range(L)]
    sqrt_iSWAP = iSwapGate().power(1 / 2)
    ...
    for i, theta in enumerate(theta_list):
        ...
        for j in range((n - offset) // 2):
            if j + offset != 2:
                ...
                circuit.cz(2 * j + offset, 2 * j + 1 + offset) # two cz that cancel each other
                circuit.cz(2 * j + offset, 2 * j + 1 + offset)
