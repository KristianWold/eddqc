...
import numpy as np
import qiskit as qk
from src import integrable_circuit_no_coupling, prepare_SPAM_strings, generate_circuit_sandwich
...
if __name__ == "__main__":
    ...
    reps = 15    # repeated experiments/ different target circuits

    circuit_list = []

    # same experiment repeated 15 times with different seeds
    for i in range(reps):
        np.random.seed(42 + i) # different seeds for each realization
        circuit_target = integrable_circuit_no_coupling(n + n_anc, L)
